...
//I haven't changed any fuctions in the file.I Only added 2 more fuctions .
//Because with multiple segmetation files we hide info
//(with handler),this is a solution to fix the occuring compiler error in evretirio file
TStoixeiouDDA AVLTree_data(typos_deikti tree)
{
	return tree->dedomena;
}

//Get the key from tree's node
