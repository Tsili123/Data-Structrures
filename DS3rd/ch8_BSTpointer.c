...
//almost same fuctions added as in AVL Tree.
//rest of file stays same

//I tried adding a condition in Tree search code
//so as not to count multiple times the same airport
//But it didn't work.I don't know if it was necessary.
TStoixeiouDDA Tree_data(typos_deikti tree)
{
	return tree->dedomena;
}


int Tree_data_key(typos_deikti tree)
{
	
	int index;
	index=TSDDA_getkey(tree->dedomena);
	return index;
}
